 ...
 */
@Controller
@RequestMapping("/control/person/manage")
public class PersonManageAction {
	private static final Logger logger = Logger.getLogger(PersonManageAction.class);
	...
	public Map<String, Object> updateVisible(String userName,int visible) throws Exception {
		...
		if (customer == null) {
			...
			PerResult result=customer.getPerResult();
			if(result!=null){
		...
		if (flag) {
			...
			PerBaseInfo info=customer.getPerBaseInfo();
			if (visible == 1&& info != null && info.getProcess() == null) { 
				judgeProccess = true; 
			}
			customerService.updatePerVisible(judgeProccess, userName, visible, info.getUuid());
			if (visible == 1) {
	 ...
	 * @desc
	 */
	@Permission(model = "person", privilegeValue = "insert")
	@RequestMapping("/addPerson")
	@ResponseBody
	public Map<String, Object> addPerson(HttpServletRequest request,Customer customer) throws Exception {
		message.setLength(0);
		flag=true;
	 ...
	 * @desc
	 */
	@Permission(model = "person", privilegeValue = "delete")
	@RequestMapping("/deletePerson")
	@ResponseBody
	public Map<String, Object> deletePerson(String userName) throws Exception {
		Customer customer=customerService.find(userName.trim());
		message.setLength(0);
		flag=true;
		if(customer==null){
		...
		if (flag) {
			...
			msgMap.put("success", true);
			msgMap.put("status", true);
			msgMap.put("msg", "删除个人用户成功");
			logger.info("删除个人用户成功");
			logger.info(moduleName + "[管理员删除:" + message + "]");
		} else {
			msgMap.put("success", true);
			msgMap.put("status", false);
			msgMap.put("msg", "删除个人用户失败");
			msgMap.put("result", message.toString());
	 ...
	 * @desc
	 */
	@Permission(model = "person", privilegeValue = "update")
	@RequestMapping("/editPersonUI")
	@ResponseBody
	public Map<String, Object> editPersonUI(String userName) throws Exception {
		Customer customer = customerService.find(userName.trim());
		if (customer != null) {
			msgMap.put("status", true);
		} else {
	...
	@ResponseBody
	public Map<String, Object> editPerson(Customer customer) throws Exception {
		message.setLength(0);
		flag=true;
	 ...
	 * @desc
	 */
	@Permission(model = "person", privilegeValue = "initPassword")
	 ...
	 * @desc
	 */
	@Permission(model = "person", privilegeValue = "appointOrganization")
