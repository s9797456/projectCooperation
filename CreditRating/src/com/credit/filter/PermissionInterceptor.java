		...
		if (handler.getClass().isAssignableFrom(HandlerMethod.class)) {
			...
			if (authPassport == null)
				...
				if (validate(handler, request)) {
					...
					response.addHeader("error:403", "true");
					return false;
				}
			}
		} else{
		...
		if (method != null && method.isAnnotationPresent(Permission.class)) {
			...
			if (permission.model().contains(",")) {
				...
				for (int i = 0; i < arr1.length; i++) {
					SystemPrivilege methodPrivilege = new SystemPrivilege(new SystemPrivilegePK(arr1[i], arr2[i]));
					Set<SystemPrivilege> sps = WebUtil.getUserSps(request);
					if(sps.contains(methodPrivilege)) return true;
				}
			} else {
				SystemPrivilege methodPrivilege = new SystemPrivilege(new SystemPrivilegePK(permission.model(), permission.privilegeValue()));
				Set<SystemPrivilege> sps = WebUtil.getUserSps(request);
				if(sps.contains(methodPrivilege)) return true;
			}
		}
