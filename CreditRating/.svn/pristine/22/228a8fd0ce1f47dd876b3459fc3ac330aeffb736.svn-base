var viewUploadFileStore = Ext.create("PM.store.enterprise.uploadFiles");
viewUploadFileStore.on('beforeload', function (store, options) {
	var t = Ext.ComponentQuery.query('viewEnterBasicInfo form textfield[name=uuid]')[0].getValue();
	var new_params = { 
			entID: t,
    };
    Ext.apply(store.proxy.extraParams, new_params);
});
var viewEntShareholderStore = Ext.create("PM.store.enterprise.entShareholderWin");
viewEntShareholderStore.on('beforeload', function (store, options) {
	var t = Ext.ComponentQuery.query('viewEnterBasicInfo form textfield[name=uuid]')[0].getValue();
	var new_params = { 
			entID: t,
    };
...
var viewEntExecutivesStore = Ext.create("PM.store.enterprise.entExecutivesWin");
viewEntExecutivesStore.on('beforeload', function (store, options) {
	var t = Ext.ComponentQuery.query('viewEnterBasicInfo form textfield[name=uuid]')[0].getValue();
	var new_params = { 
			entID: t,
	};
...
Ext.define('PM.view.enterprise.viewEnterBasicInfo',{
	extend:'Ext.Window',
	alias:"widget.viewEnterBasicInfo",
	layout : 'anchor',
	border : false,
	width : 1250,
	height : 650,
					   	...
					   	dockedItems : [ {
							...
							items : {
								...
								onTrigger2Click : function(obj,e){ // 节点单击事件监听
									...
									var scanFiles = Ext.ComponentQuery.query("viewEnterBasicInfo tabpanel grid#uploadFilesGird")[0];
									scanFiles.getStore().load({
										params : {
											entID: Ext.ComponentQuery.query("viewEnterBasicInfo tabpanel form textfield[name=uuid]")[0].getValue(),
											name : name
										}
									});
								}
