...
function readProtocols(state,name){
	var str = "";
	if(state != 1){
		str = "<button type='button' class='btn btn-primary' data-dismiss='modal'  style= 'margin-left: 92%;margin-top: 10px;' id='agree'>同意</button>";
	}else{
		str = "<button type='button' class='btn btn-primary' data-dismiss='modal' style= 'margin-left: 92%;margin-top: 10px;'>关闭</button>";
	}
	$("body").append("<div class='modal fade' id='readinfo' style='top: 2%'>"
	...
	$("#agree").on("click",function(){
		$.ajax({
			type : "post",
			url : _ctxPath + "/initPersonal/readProtocols.do",
...
function rejectedItem(opinion){
	$.ajax({
        cache: true,
        type: "POST",
        url:_ctxPath+'/initPersonal/opinion.do',
        async: false,
        error: function() {
        ...
        success: function(data) {
        	if(data.status){
        		...
        		$("body").append("<div class='modal fade' id='reject' style='top: 20%'><div class='modal-dialog '>"
...
function downloadReport(){
	$.ajax({
        cache: true,
        type: "POST",
        url:_ctxPath+'/initPersonal/downloadReport.do',
        async: false,
        error: function() {
            popup("网络错误，请重新登录    错误代码：400");
        },
        success: function(data) {
        	if(data.status){
        		...
        		var encoding = data.perResult.encoding;
        		if(encoding == null) {
        			encoding = " - </span>"+c;
        			flag = false;
        		}else{
        		...
        		var gradetime = data.perResult.gradetime;
        		if(gradetime == null) {
        			gradetime = " - </span>"+c;
        			flag = false;
        		}else{
        		...
        		if(data.reporturl == 1 && flag == true){
        			span = "检查完成</span>"+d ;
        			dow = "<button type='button' class='btn btn-primary' id='download'>下载</button>";
        		}
        		$("body").append("<div class='modal fade' id='downloadReport' style='top: 20%'><div class='modal-dialog'>"
            			+"<div class='modal-content'><div class='modal-header'>"
            			   +"<button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>"
            			   +"<h4 class='modal-title'>下载报告</h4></div>"
            			   +"<div class='modal-body'>" 
            			   +"<div style = 'width: 80%; margin:10px auto;'> <span> 姓名&nbsp;&nbsp; ： "+data.perBaseInfoName+"</span><span class = 'glyphicon glyphicon-ok-sign' style = 'color: rgb(62, 182, 62); float: right; font-size: 20px;'></span></div>"
            	...
            	$("body").on("click","#download",function(){
            		window.location.href= _ctxPath+'/initPersonal/downLoad.do';
            		$("#downloadReport").modal('hide');
            	});
        	}else{
        		popup("登陆超时，请重新登录    错误代码: 101 ");
        	}
        }
    });
...
function historical(historical){
	historical= 1;
	if(historical == 1 ){
		window.location.href= _ctxPath+'/initPersonal/historical.do';
	}else{
		popup("暂无历史内容");
		return;
	}
...
function downloadReport_historical(hisId){
	...
	$.ajax({
        ...
        type: "POST",
        data:{ "hisId":hisId},
        url:_ctxPath+'/initPersonal/downloadReportHistoricalUI.do',
        ...
        success: function(data) {
        	if(data.status == 1){
        		...
        		$("body").append("<div class='modal fade' id='downloadHistorical' style='top: 20%'><div class='modal-dialog'>"
            			+"<div class='modal-content'><div class='modal-header'>"
            			   +"<button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button>"
            	...
            	$("body").on("click","#downloadHis",function(){
            		window.location.href= _ctxPath+'/initPersonal/downLoadHistorical.do?hisId='+hisId;
